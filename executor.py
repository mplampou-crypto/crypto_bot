"""Bybit V5 execution layer (linear perpetuals) via pybit. Synchronous;
called from the async bot through run_in_executor."""
import logging
...
class Executor:
    ...
    def qty_step(self, symbol):
        return self.instrument(symbol)["qty_step"]
    ...
    def position_size(self, symbol):
        """Signed Bybit position: + long, - short, 0 flat (Decimal)."""
    ...
    def equity(self):
        try:
            ...
            acc = r["result"]["list"][0]
            return float(acc.get("totalEquity") or 0), \
                float(acc.get("totalAvailableBalance") or 0)
    ...
    def configure_symbol(self, symbol, leverage):
        try:
            self.session.switch_position_mode(
                category="linear", symbol=symbol, mode=0)  # one-way
        except Exception as e:
        ...
        try:
            ...
            if "not modified" not in str(e).lower() and "110043" not in str(e):
                log.warning("set leverage (%s -> %sx): %s", symbol, leverage, e)

    # ── orders ──────────────────────────────────────────────────
    def market(self, symbol, side, qty):
        """Market order. side='Buy'|'Sell', qty=Decimal>0."""
